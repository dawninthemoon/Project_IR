Shader "AllIn1SpriteShader/AllIn1SpriteShaderScaledTime"
{
    Properties
    {
		...
		_OutlineAlpha("Outline Base Alpha",  Range(1,1)) = 1 //15
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			float sobel(sampler2D tex, float2 uv) {
				...
				hr += tex2D(tex, (uv + float2(-1.0, -1.0) * delta)).a * 1.0;
				hr += tex2D(tex, (uv + float2(0.0, -1.0) * delta)).a * 0.0;
				hr += tex2D(tex, (uv + float2(1.0, -1.0) * delta)).a * -1.0;
				hr += tex2D(tex, (uv + float2(-1.0, 0.0) * delta)).a * 2.0;
				hr += tex2D(tex, (uv + float2(0.0, 0.0) * delta)).a * 0.0;
				hr += tex2D(tex, (uv + float2(1.0, 0.0) * delta)).a * -2.0;
				hr += tex2D(tex, (uv + float2(-1.0, 1.0) * delta)).a * 1.0;
				hr += tex2D(tex, (uv + float2(0.0, 1.0) * delta)).a * 0.0;
				hr += tex2D(tex, (uv + float2(1.0, 1.0) * delta)).a * -1.0;

				vt += tex2D(tex, (uv + float2(-1.0, -1.0) * delta)).a * 1.0;
				vt += tex2D(tex, (uv + float2(0.0, -1.0) * delta)).a * 2.0;
				vt += tex2D(tex, (uv + float2(1.0, -1.0) * delta)).a * 1.0;
				vt += tex2D(tex, (uv + float2(-1.0, 0.0) * delta)).a * 0.0;
				vt += tex2D(tex, (uv + float2(0.0, 0.0) * delta)).a * 0.0;
				vt += tex2D(tex, (uv + float2(1.0, 0.0) * delta)).a * 0.0;
				vt += tex2D(tex, (uv + float2(-1.0, 1.0) * delta)).a * -1.0;
				vt += tex2D(tex, (uv + float2(0.0, 1.0) * delta)).a * -2.0;
				vt += tex2D(tex, (uv + float2(1.0, 1.0) * delta)).a * -1.0;
			...
			half4 GetPixelRGBA(in int offsetX, in int offsetY, half2 uv, sampler2D tex)
			{
				return tex2D(tex, (uv + half2(offsetX * _MainTex_TexelSize.x, offsetY * _MainTex_TexelSize.y)));
			}

			float getSobelEdge(float2 uv)
			{
				float gradientX = 0.0;
				float gradientY = 0.0;
				for (int i = -1; i <= 1; i++)
				{
					for (int j = -1; j <= 1; j++)
					{
						float alpha = GetPixelRGBA(j, i, uv, _MainTex).a;
						gradientX += alpha * sobelKernelX[(i + 1) * 3 + (j + 1)];
						gradientY += alpha * sobelKernelY[(i + 1) * 3 + (j + 1)];
					}
				}
            ...
            half4 frag (v2f i) : SV_Target
            {
				...
				col.rgb = length(innerT) * _InnerOutlineColor.rgb * _InnerOutlineGlow;
				col.a = 1.0f - step(0.2f, col.r + col.g + col.b);
				#endif
				#endif
				...
				//OUTLINE-------------------------------------------------------------
					...
					result *= (1.0f - originalAlpha) * _OutlineAlpha;
				...
				* max(sign((_ShineLocation + _ShineWidth) - currentDistanceProjection), 0.0) * _ShineColor * shineMask;
				...
				//float sobel = getSobelEdge(i.uv);

				col *= _Color;

				//if (sobelEdge >= 0.00001f)
				//{
				//	col.a = 0.0f;
				//}
				//col.a *= col.a / sobelEdge;
